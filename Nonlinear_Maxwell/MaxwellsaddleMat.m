function [eqn,info] = MaxwellsaddleMat(node,elem,bdFlag,pde,option,varargin)
...
%% Poisson stiffness matrix
[Dphi,volume] = gradbasis3(node,elem);
K = sparse(N,N);
for i = 1:4
    for j = i:4
        Kij = dot(Dphi(:,:,i),Dphi(:,:,j),2).*volume;
        if (j==i)
            K = K + sparse(elem(:,i),elem(:,j),Kij,N,N);
        else
...
%% Boundary conditions
f0 = f;
[AD,f,u,isFreeNode,isFreeEdge] = getbdMaxwellsaddle(f0);
p  = zeros(N,1);
g0 = g0 - G'*u;

%% Output
StifMat = struct('ii',ii,'jj',jj,'sA',sA,'sM',sM,'diagIdx',diagIdx,'upperIdx',upperIdx);
eqn = struct('StifMat',StifMat,'G',G,'f',f,'f0',f0, 'gall', gall, 'g',g0,'M',M,'K',K,'AD',AD,'grad',grad,...
