function [x,info] = mgMaxwell_1(A,b,node,elem,edge,HBmesh,option)
%% MGMAXWELL multigrid-type solver for Maxwell equations, adapted for tensor coefficient
% 
...
% extend Pro and Res to vector S^1 space
for i = 1:level-1
    ProA{i} = kron(speye(3), Pro{i});
    ResA{i+1} = kron(speye(3), Res{i+1});
end

APi = cell(level,1);
BPi = cell(level,1);
TbdA = spdiags(repmat(bdidx,3,1),0,3*N,3*N);
APi{level} = IIt*A*II+TbdA;    

BPi{level} = BP;                
for j = level:-1:2
    APi{j-1} = ResA{j}*APi{j}*ProA{j-1};           % Ac = Res*Af*Pro    
...
%     eh = triu(A)\(D.*(tril(A)\r));  % Gauss-Seidal. less iteration steps
    ...
    % Part1: II*(AP)^{-1}*II^t
    rc = IIt*r(1:size(IIt,2));   % transfer to the nodal linear element space
    ri = cell(level,1);        
    ei = cell(level,1);        
    ri{level} = rc;            
    for i = level:-1:2
        ei{i} = Ri{i}\ri{i};   
        ri{i-1} = ResA{i}*(ri{i}-APi{i}*ei{i});
    end
    ei{1} = APi{1}\ri{1};  
    for i = 2:level
        ei{i} = ei{i} + ProA{i-1}*ei{i-1};
