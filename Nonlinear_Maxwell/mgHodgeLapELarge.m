function [x,info,Ai,Bi,BBi,Res,Pro] = mgHodgeLapELarge(A,b,node,elem,bdFlag,option,varargin)
...
switch solver
   case 'CG'
        ...
        while (max(err(k,1)) >= tol*max(err(2,1)) || k<=2) && (k <= maxIt) %(max(err(k,:)) > tol) && (k <= maxIt)    
            % compute Br by MG
            Br = vcycle(r);
