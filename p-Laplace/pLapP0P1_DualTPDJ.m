function [sigma,u,eqn,info] = pLapP0P1_DualTPDJ(node,elem,bdFlag,pde,option)
...
%     fprintf('Iteration %i \n', ite);
     ...
     Jinv = pde.Jsigmainv(sigmaoldAll, teps, eps0) ;
     MJinv = [1./area; 1./area].*Jinv;

     % Jepsinv = Jinv + 1000*speye(size(Jinv));
     % MJinv = [1./area; 1./area].*Jepsinv;
     ...
     ruall = zeros(N,1);
     ru =  ru - eqn.Bff*( MJinv*rsigma);
     ruall(isfreeNode) = ru;
     % 
     % if  ite > 2 && res(ite-1) < 1e-3*res0
     %     stepsizes = 1;
     % elseif mod(ite, 30) == 0
     %     stepsizes = 1.3*stepsizes;
...
%      % multigrid
     ...
     dsigma = MJinv*(rsigma+eqn.Bfft*duAll(isfreeNode));

     sigmanewAll = sigmaoldAll - stepsizes*reshape(dsigma,NT, 2);
