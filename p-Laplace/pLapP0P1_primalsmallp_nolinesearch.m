function [sigma,u,eqn,info] = pLapP0P1_primalsmallp_nolinesearch(node,elem,bdFlag,pde,option)
...
if ~isempty(pde.Ku) && ~isnumeric(pde.Ku)       % d is a function
    ...
    gradunormeps =  gradunorm + keps;
    K = pde.Ku(gradunorm ); % K is a scalar
    Keps = pde.Ku(gradunormeps ); 
end
...
Apeps = B*([Keps; Keps]./[area;area].*Bt);
...
%      % multigrid
    ...
    %  gradudu = reshape(grad*(uoldAll+stepsize*du), NT, 2);
    %  graddu = reshape(grad*du, NT, 2);
    %  % Assemble Mass Matrix
    %  if ~isempty(pde.Ku) && ~isnumeric(pde.Ku)       % d is a function
    %     gradudunorm = sqrt(sum(gradudu.^2, 2));
    %     K = pde.Ku(gradudunorm); % K is a scalar
    %     % J = pde.Jdu(gradudunorm).*sum(graddu.*gradudu, 2); % K is a scalar
    % end
    % Apdu = B*([K; K]./[area;area].*Bt);
    % % ApJdu = B*([J; J]./[area;area].*Bt);
    % r = du'*(Apdu*(uoldAll+stepsize*du)) - du'*f;
    % in_ite = 0;
    % 
    % while abs(r) > 1e-5 && in_ite < 20
    %      % s = du'*(Apdu*du) + (p-2)*(du'*(ApJdu*(uoldAll+stepsize*du)));
    %      % stepsize = stepsize - 0.2*r/s;
    %      stepsize = stepsize - 0.2*r;
    %      disp(r)
    % 
    %      r = du'*(Apdu*(uoldAll+stepsize*du)) - du'*f;
    % 
    ...
    %         gradudunorm = sqrt(sum(gradudu.^2, 2));
    %         K = pde.Ku(gradudunorm); % K is a scalar
    %         % J = pde.Jdu(gradudunorm).*sum(graddu.*gradudu, 2); 
    %     end
    %     Apdu = B*([K; K]./[area;area].*Bt);
    %     % ApJdu = B*([J; J]./[area;area].*Bt);
    %     in_ite = in_ite +1;
